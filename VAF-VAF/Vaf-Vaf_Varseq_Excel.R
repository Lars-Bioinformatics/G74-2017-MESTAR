...
# install.packages("tidyverse","maftools","parallel","ggiraph","htmlwidgets","data.table")
...
snv_caller = "varscan2_varseq_excel"

cnv_caller = "sequenza"


# Choose output folder
output = "output/connor_pipeline/VAF_VAF_plots/"
# output = "VAF_VAF_plots/"
...
join_vaf_vs_vaf <- function(merged){
    data = merged %>%
        ...
        # mutate(sample_group=str_sub(sample, 1, 5)) %>%
        inner_join(., ., by=c("chr", "start", "end", "sample_group", "Hugo_Symbol")) %>%
        filter(sample.x < sample.y) %>%
        mutate(tooltip=paste0("Gene: ", Hugo_Symbol, 
                              "\n", sample.x," CNV: ", tumor.total.x, "-", tumor.major.x, "-", tumor.minor.x, " SNV: ", t_alt_count.x, "-", t_ref_count.x,
                              "\n", sample.y," CNV: ", tumor.total.y, "-", tumor.major.y, "-", tumor.minor.y, " SNV: ", t_alt_count.y, "-", t_ref_count.y))
    return(data)
}
...
run_vaf_vaf = function(){
    ...
    # cl <- makeCluster(2)
    clusterExport(cl, varlist = c("merged", "filter_variants", "join_vaf_vs_vaf", "vaf_vaf_combined_sample", "output", "snv_caller",
    ...
    parApply(cl, filters_config, 1, function(filters){
        ...
        output2 = paste0(output, snv_caller, "/", snv_caller ,extra,"/")
