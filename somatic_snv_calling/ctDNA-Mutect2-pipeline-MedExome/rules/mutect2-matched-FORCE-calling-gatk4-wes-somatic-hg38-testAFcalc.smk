...
'''
rule Mutect2_matched_calling_forced_testAFcalc:
    input:
    ...
    output:
        vcf="{output}vcf_files_testAFcalc/{tumor}_vs_{normal}_somatic_mutect2-forced.vcf.gz",
        idx="{output}vcf_files_testAFcalc/{tumor}_vs_{normal}_somatic_mutect2-forced.vcf.gz.tbi",
        # realigned_bam="{output}realigned_bam/{tumor}_vs_{normal}_somatic_mutect2-forced.bam",
        vcf_stats="{output}vcf_files_testAFcalc/{tumor}_vs_{normal}_somatic_mutect2-forced.vcf.gz.stats",
        f1r2="{output}f1r2_testAFcalc/{tumor}_vs_{normal}_f1r2.tar.gz"
...
rule FilterMutectCalls_forced_testAFcalc:
    input:
        vcf="{output}vcf_files_testAFcalc/{sample}_somatic_mutect2-forced.vcf.gz",
        idx="{output}vcf_files_testAFcalc/{sample}_somatic_mutect2-forced.vcf.gz.tbi",
        # bam="{output}realigned_bam/{sample}_somatic_mutect2-forced.bam",
        stats="{output}vcf_files_testAFcalc/{sample}_somatic_mutect2-forced.vcf.gz.stats",
    ...
    output:
        vcf="{output}vcf_filterFlag_testAFcalc/{sample}_somatic_mutect2-forced_filterFlag.vcf.gz",
        tbi="{output}vcf_filterFlag_testAFcalc/{sample}_somatic_mutect2-forced_filterFlag.vcf.gz.tbi",
    shell:
        """
...
rule vcf_norm_decompose_bcftools_testAFcalc:
    input:
        vcf="{output}vcf_filterFlag_testAFcalc/{sample}_somatic_mutect2-forced_filterFlag.vcf.gz"
    output:
        norm_vcf="{output}vcf_filterFlag_norm_testAFcalc/{sample}_somatic_mutect2-forced_filterFlag_norm.vcf.gz"
    shell:
        """
...
rule intersect_forcedVCF_mergedVCF_testAFcalc:
    input:
        norm_vcf="{output}vcf_filterFlag_norm_testAFcalc/{tumor}_vs_{normal}_somatic_mutect2-forced_filterFlag_norm.vcf.gz",
        force_intervals=lambda wildcards: expand(main_output+"union_vcf/{patient}"+force_intervals_suffix, patient=wildcards.normal[:5]+wildcards.normal[7:])
    output:
        final_vcf="{output}vcf_final_testAFcalc/{tumor}_vs_{normal}__somatic_mutect2-forced_filterFlag_norm_final.vcf.gz"
    shell:
        """
