...
rule Mutect2_joint_calling:
    input:
        normal=lambda wildcards: expand("bam/{normal}"+bam_suffix, normal=config[wildcards.sample]["normal"]),
        tumors=lambda wildcards: expand("bam/{tumor}"+bam_suffix, tumor=config[wildcards.sample]["tumor"]),
    ...
    params:
        tumors=lambda wildcards: expand("-I bam/{tumor}"+bam_suffix, tumor=config[wildcards.sample]["tumor"]),
